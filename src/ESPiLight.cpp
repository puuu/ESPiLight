...
void ICACHE_RAM_ATTR ESPiLight::interruptHandler()
{
  ...
  {
    unsigned long now = micros();
    unsigned int duration = now - _lastChange;
...
ESPiLight::ESPiLight(int8_t outputPin)
{
  ...
  protocols_t *pnode = get_protocols(); // Load protocols here
  // Check if we need to adjust minrawlen, maxrawlen, mingaplen, maxgaplen, minPulseLen or maxPulseLen
  while (pnode != nullptr)
  {
    ...
    if (maxLen > maxrawlen && maxLen <= MAXPULSESTREAMLENGTH)
    {
      maxrawlen = maxLen;
    }
    ...
    if (minGap < minPulseLen)
    {
      minPulseLen = minGap;
    }

    if (maxGap > maxPulseLen)
    {
      maxPulseLen = maxGap;
    }
